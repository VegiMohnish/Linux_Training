...
int thread_function1(void *pv)
{
    ...
    while(!kthread_should_stop()) {
        ...
        my_global_variable++;
        pr_info("Inside the Thread Function1 %lu\n", my_global_variable);
...
int thread_function2(void *pv)
{
    while(!kthread_should_stop()) {
        spin_lock(&my_spinlock);
        my_global_variable++;
        pr_info("Inside the Thread Function2 %lu\n", my_global_variable);
        spin_unlock(&my_spinlock);
        msleep(1000);
    }
    return 0;
...
static int __init my_driver_init(void)
{
        ...
        /* Creating Thread 1 */
        my_thread1 = kthread_run(thread_function1,NULL,"my Thread1");
        if(my_thread1) {
         ...
         /* Creating Thread 2 */
        my_thread2 = kthread_run(thread_function2,NULL,"my Thread2");
        if(my_thread2) {
